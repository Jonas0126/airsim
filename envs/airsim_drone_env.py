...
class AirSimDroneEnv(gym.Env):
    ...
    def __init__(self, client, drone_id, max_episode_steps=1000, step_length=1):
        super().__init__()
        MAX_ROLL_RATE = 3.0
        MAX_YAW_RATE = 3.0
        MAX_PITCH_RATE = 3.0
    ...
    def _setup_flight(self):
        # self.drone.reset()
        # self.drone.enableApiControl(True, vehicle_name=self.drone_id)
        # self.drone.armDisarm(True, vehicle_name=self.drone_id)
        # print(f'reset : {self.drone_id}')

        start_pos = airsim.Vector3r(0, 0, -1.46)
        start_rot = airsim.to_quaternion(0, 0, 0)
        self.client.simSetVehiclePose(airsim.Pose(start_pos, start_rot),
                                ignore_collision=True,
                                vehicle_name=self.drone_id)
    def _get_obs(self):
        self.drone_state = self.client.getMultirotorState(vehicle_name=self.drone_id)
        ...
        # concatenate
        obs = np.concatenate([p, v, rot_mat, self.prev_action])


        # collision state
        collision = self.client.simGetCollisionInfo(vehicle_name=self.drone_id).has_collided
